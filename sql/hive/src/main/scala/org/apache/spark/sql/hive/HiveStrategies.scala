...
class ResolveHiveSerdeTable(session: SparkSession) extends Rule[LogicalPlan] {
  private def determineHiveSerde(table: CatalogTable): CatalogTable = {
    if (table.storage.serde.nonEmpty) {
      ...
      val defaultStorage =
        HiveSerDe.getDefaultStorage(session.sessionState.conf)
        ...
        HiveSerDe.sourceToSerDe(options.fileFormat.get) match {
          ...
          case None =>
            throw new IllegalArgumentException(
              s"invalid fileFormat: '${options.fileFormat.get}'"
            )
        }
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan transformUp {
    case c@CreateTable(t, _, query) if DDLUtils.isHiveTable(t) =>
      ...
      val table =
        t.copy(identifier = t.identifier.copy(database = Some(dbName)))
...
class DetermineTableStats(session: SparkSession) extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = plan transformUp {
    case relation: HiveTableRelation
      if DDLUtils
        .isHiveTable(relation.tableMeta) && relation.tableMeta.stats.isEmpty =>
...
case class RelationConversions(conf: SQLConf,
  ...
  private def convert(relation: HiveTableRelation): LogicalRelation = {
    ...
    if (serde.contains("parquet")) {
      val options = relation.tableMeta.storage.properties + (ParquetOptions.MERGE_SCHEMA ->
        conf
          .getConf(HiveUtils.CONVERT_METASTORE_PARQUET_WITH_SCHEMA_MERGING)
          .toString)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    plan transformUp {
      ...
      // Read path
      case relation: HiveTableRelation
        if DDLUtils.isHiveTable(relation.tableMeta) && isConvertible(
          relation
        ) =>
        convert(relation)
    }
...
private[hive] trait HiveStrategies {
  ...
  object HiveTableScans extends Strategy {
    ...
    private val stringConverter =
      CatalystTypeConverters.createToScalaConverter(StringType)

    private type ReplaceMap = mutable.HashMap[String, AttributeReference]
    private type ReplaceFunc =
      (Expression, ReplaceMap, HiveTableRelation) => Option[AttributeReference]
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      relation: HiveTableRelation
        ...
        if (sparkSession.sparkContext.conf
          .getBoolean("spark.sql.json.optimize", false)) {
    ...
    private def mkAttribute(function: String,
      ...
      val metadata = new MetadataBuilder()
        ...
        .build()
      attrMap.getOrElseUpdate(
        key,
        AttributeReference(attrName, StringType, metadata = metadata)(
    ...
    private def replaceJson(
      ...
      expr match {
        case GetJsonObject(r: AttributeReference, Literal(v, StringType)) =>
          ...
          if (ReadJson.jsonPathExists(
            sparkSession,
            relation.tableMeta.database + "_" + relation.tableMeta.identifier.table,
            r.name + "_" + path
          )) {
    ...
    private def collectJsonInfo(
                                 projectList: Seq[NamedExpression],
                                 filterPredicates: Seq[Expression],
                                 relation: HiveTableRelation
                               ): (mutable.Set[JsonInfo], mutable.Set[JsonInfo]) = {
    ...
    private def collect(expr: Expression,
                        set: mutable.Set[JsonInfo],
                        relation: HiveTableRelation): Unit = {
      expr.foreach {
        case GetJsonObject(r: AttributeReference, Literal(v, StringType)) =>
          set.add((relation, r, stringConverter(v).asInstanceOf[String]))
        case _ => //do nothing
      }
    }
