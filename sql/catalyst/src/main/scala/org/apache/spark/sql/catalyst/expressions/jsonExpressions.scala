  ...
  examples = """
    Examples:
      > SELECT _FUNC_('{"a":"b"}', '$.a');
       b
  """)
case class GetJsonObjectMison(json: Expression, path: Expression)
  extends BinaryExpression with ExpectsInputTypes with CodegenFallback {
  ...
  override def dataType: DataType = StringType
  override def nullable: Boolean = true
  override def prettyName: String = "get_json_object_mison"

  private lazy val parsedPath2 = path.eval().asInstanceOf[UTF8String]

  override def eval(input: InternalRow): Any = {
    val parsed = if (path.foldable) {
      parsedPath2
    } else {
      path.eval(input).asInstanceOf[UTF8String]
    }
    UTF8String.fromBytes(
      JsonExprUtils.misonParse(
        parsed.getBytes,
        json.eval(input).asInstanceOf[UTF8String].getBytes
      ))
  }
}

// scalastyle:on line.size.limit
case class JsonTupleMison(children: Seq[Expression])
  ...
  override def prettyName: String = "json_tuple_mison"
...
//  System.load("/home/feiwang/asplos/rust/pikkr/target/release/libpikkr.so")
  System.load("c:\\Users\\zyp\\ali\\rust\\pikkr\\target\\release\\pikkr.ddl")
  ...
  def misonParse(queryStr: Array[Byte], record: Array[Byte]): Array[Byte] = {
    parse(queryStr, record)
  }
}
